...
4. Convert figure references to MyST figure directives using the exact
   path from the file inventory — do not add or assume a `figures/`
   prefix unless the file is actually listed under `figures/` in the
   inventory:
   :::{figure} [exact-path-from-inventory]
   :label: fig-[slug]
   :width: 75%
   [Caption from source]
   :::
...
license: CC-BY-4.0
thumbnail: [exact path from inventory of the most representative figure]
collections:
